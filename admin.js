document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchFileFromGithub(path) {
    ...
    if (!owner || !repo || !token) {
      throw new Error("Settings missing: Please check GitHub Configuration.");
    }
    ...
    try {
      ...
      // SAFE PARSE: Remove invisible characters (BOM, etc.) from the start/end
      ...
      try {
        ...
        throw new Error(`JSON Formatting Error: The file on GitHub has a typo or hidden character. (Error: ${parseErr.message})`);
      }
    } catch (e) {
  ...
  async function pushFileToGithub(path, contentObj, sha, commitMsg) {
    ...
    if (response.status === 401) throw new Error("Invalid Token: Unauthorized push.");
    if (!response.ok) {
      const errData = await response.json().catch(() => ({}));
      throw new Error(`Push Error (${response.status}): ${errData.message || 'Unknown error'}`);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (!response.ok) {
             const errData = await response.json().catch(() => ({}));
             throw new Error("Image upload failed: " + (errData.message || "Unknown error"));
          }
          resolve(jsonPath);
        } catch (err) {
          reject(err);
